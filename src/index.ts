 ...
 * @param that 
 */
function getHookLife(that: any): hookLife {
    let id = that._life_id_
    if (!id) {
        lifeIndexNum += 1
        id = that._life_id_ = "$" + lifeIndexNum.toString()
    }
    let life: hookLife = hookLifes[id]
    if (!life) {
        let data = {}
 ...
 * @param vueLifeName 
 */
function addHookLifes(that: any, vueLifeName: string): hookLife {
 ...
 * @param key 
 */
function hookEmitEvent(life: hookLife, key: string): hookEvent {
    return {
        data: life.data[key],
        /**
         * 当前vm中触发新的事件
...
function _hookExec(key: string, life: hookLife, data?: any):void {
    if (!data) {
        return
    }
...
export function install(Vue: VueConstructor, init: Function | initObj) {
    // 初始化函数
